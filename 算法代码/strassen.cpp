...
int main()
{
    StrassenClass<int> stra;
    int Size = 0;
    int** MatrixA;
    int** MatrixB;
    int** MatrixC;
    cout<<"请输入矩阵大小(2的幂指数值)"<<endl;
    cin>>Size;
    int N = Size;
    ...
    stra.FillMatrix(MatrixA,MatrixB,Size);
    cout<<"矩阵A的值为:"<<endl;
    stra.PrintMatrix(MatrixA,Size);
    cout<<"矩阵B的值为:"<<endl;
    stra.PrintMatrix(MatrixB,Size);

    cout<<endl<<"矩阵的运算结果为："<<endl;
    stra.Strassen( N, MatrixA, MatrixB, MatrixC );
    stra.PrintMatrix(MatrixC,Size);
    return 0;
}
